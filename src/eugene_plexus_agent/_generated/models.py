...
class EngineKind(StrEnum):
    ...
    safetensors, on Apple silicon only. We never ship an engine —
    every one of them is an upstream project we wrap and track.
    ...
    again and the most awkward: it serves HTTP immediately and its
    `/health` answers a hardcoded `{"status": "ok"}` while the
    model is still loading on another thread, so **no read-only
    probe can tell loading from ready**. Its adapter proves
    residency by asking for one token instead. They differ in
    acquisition too: a llama.cpp build is fetched and verified by
    us, while vLLM and mlx-lm are Python packages the operator
    installs themselves. See `EngineAcquisition.policy`.
